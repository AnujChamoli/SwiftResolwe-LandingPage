...
const TICKER_ITEMS = [
  "Modular Workflows: Standalone or Sequential Negotiation, Mediation, Arbitration",
...
const MetricCard = ({ value, suffix, label, decimals = 0, testId }) => {
  ...
  return (
    <div
      ref={ref}
      data-testid={testId}
      className="relative card-light p-8 sm:p-10"
    >
...
const PlaceholderLogo = ({ name, type, mark }) => (
  <div
    ...
    className="flex items-center gap-3 px-5 py-3 rounded-xl border border-[var(--border-soft)] bg-[var(--bg-surface)] opacity-75 hover:opacity-100 transition"
  >
    <span className="inline-flex items-center justify-center w-8 h-8 rounded-md bg-[var(--bg-surface-2)] text-[var(--text-secondary)] font-mono-ui text-[10px]">
      {mark}
    </span>
    <div className="leading-tight">
      ...
      <div className="font-mono-ui text-[9.5px] tracking-[0.2em] text-[var(--text-muted)] uppercase">
        {type}
      </div>
    </div>
  </div>
...
export const TrustAnchor = () => {
  return (
    <section
      ...
      className="relative py-20 sm:py-24 bg-[var(--bg-surface-2)] border-y border-[var(--border-soft)] overflow-hidden"
    >
      <div className="relative max-w-7xl mx-auto px-5 sm:px-8">
        {/* Metrics */}
        <div className="grid sm:grid-cols-3 gap-4 sm:gap-6">
          <MetricCard testId="metric-resolution-time" value={45} suffix=" Days" label="Avg Resolution Time" />
          <MetricCard testId="metric-cost-reduction" value={90} suffix="%" label="Cost Reduction" />
          <MetricCard testId="metric-enforceability" value={100} suffix="%" label="Statutory Enforceability" />
        </div>

        {/* Ticker (deep slate stripe) */}
        <motion.div
          initial={{ opacity: 0 }}
          ...
          data-testid="ticker"
          className="marquee mt-14 border-y border-[#1F2A3E] py-5 bg-[var(--bg-deep)] rounded-xl"
        >
          <div className="marquee-track">
            {[...TICKER_ITEMS, ...TICKER_ITEMS].map((t, i) => (
              <div key={i} className="flex items-center gap-6 px-6">
                <span className="inline-block w-1.5 h-1.5 rounded-full bg-[var(--accent-bright)]" />
                <span className="font-mono-ui text-[0.78rem] tracking-[0.12em] text-[var(--text-on-dark)] uppercase whitespace-nowrap">
                  {t}
                </span>
              </div>
            ))}
